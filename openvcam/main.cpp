...
int wmain (int argc, wchar_t** argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (param.find(L"/i:") != wstring::npos) {
            input_video_ids = param.substr(3);
        }
        else if (param.find(L"/m:") != wstring::npos) {
            meta_data = param.substr(3);
        }
        else if (param.find(L"/t:") != wstring::npos) {
            saved_model = param.substr(3);
        }
        else if (param.find(L"/d") != wstring::npos) {
            display_window = true;
        }
        else if (param.find(L"/o") != wstring::npos) {
            create_video_only = true;
        }
    }

    if ((input_video_ids != L"") && (meta_data != L"") && (saved_model != L""))
    {
        wcout << endl;
        wcout << L"---------------------------------------------------------";
        wcout << L"---------------------------------------------------------";
        ...
        wcout << L"YOLO model         : " << saved_model << endl;
        wcout << L"Meta data file     : " << meta_data << endl;
        wcout << L"---------------------------------------------------------";
        wcout << L"---------------------------------------------------------";
        wcout << endl;

        int idx = StartCreateVideoMetaData(input_video_ids.c_str(), meta_data.c_str(), saved_model.c_str(), UpdateStatus);
        if (idx != 0) {
    ...
    else
    {
        ...
        wcout << L"" << endl;
        wcout << L"Usage:" << endl;
        wcout << L"openvcam /i:[input video ids] /m:[meta data] /t:[YOLO saved model] /d /o" << endl;
        wcout << L"" << endl;
